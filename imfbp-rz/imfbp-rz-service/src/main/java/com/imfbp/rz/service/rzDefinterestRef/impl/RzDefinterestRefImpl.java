package com.imfbp.rz.service.rzDefinterestRef.impl;
...
import com.imfbp.rz.service.rzDefinterestRef.RzDefinterestRef;
...
public class RzDefinterestRefImpl implements RzDefinterestRef{
	// @Autowired
	...
	private final static Logger logger = Logger.getLogger(RzDefinterestRefImpl.class);
		...
		try {
			if (refMetaDataService != null) {
				...
				if (refType.equals(IRZConsts.LIST)) {
					...
					String countSql = getQueryDataCountSql(tableName, condition, refBaseQuery,idItem);
	...
	public String getQueryDataSql(RefMetaDataService refMetaDataService, RefBasePage refBasePage,
		...
		if (refMetaDataService != null) {
		    ...
		    //通过前台传一个state的参数，修改或者查看的时候不需要条件翻译
			String state[]=refBaseQuery.getQueryCondition().split("=");
			List<String>contrPks=null;
			if(state[1].equals("'add'")){
				contrPks=rzDefInterestService.getContrByDef(new RzDefInterestQuery());
			 }else contrPks=rzDefInterestService.getContrByDefEdit(new RzDefInterestQuery());
			if(ToolUtils.isNotEmptyCollection(contrPks)){
				sql.append(" and "+ToolUtils.humpToLine(idItem)+" in (");
				int index=0;
				for(String pk :contrPks){
					if(StringUtils.isNotEmpty(pk) && index !=contrPks.size()-1 ){
						sql.append("'"+pk+"'"+",");
					}else sql.append("'"+pk+"'"+")");
					index++;
				}
				sql.append(" and contr_status=1");
			}else return null;
			}
			if (!StringUtil.isEmpty(order)) {
				sql.append(" " + order + " ");
	 ...
	 * @return
	 */
	public String getQueryDataCountSql(String tableName, String condition, RefBaseQuery refBaseQuery,String idItem) {
		// 如果是树型参照则不需要分页,如果支持分页，需要根据参数查看当前
		...
		if (!StringUtil.isEmpty(refBaseQuery.getQueryCondition())) {
			...
			if(ToolUtils.isNotEmptyCollection(contrPks)){
				countSql.append(" and "+ToolUtils.humpToLine(idItem)+" in (");
				int index=0;
				for(String pk :contrPks){
					if(StringUtils.isNotEmpty(pk) && index !=contrPks.size()-1 ){
						countSql.append("'"+pk+"'"+",");
					}else countSql.append("'"+pk+"'"+")");
					index++;
				}
			}else return null;
			}
