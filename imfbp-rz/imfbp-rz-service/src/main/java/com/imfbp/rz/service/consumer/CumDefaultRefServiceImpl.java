package com.imfbp.rz.service.consumer;
 ...
 *
 */
@Component("comDefaultRpcService")
public class CumDefaultRefServiceImpl implements RefDataService {
	...
	private final static Logger logger = Logger.getLogger(CumDefaultRefServiceImpl.class);
		...
		try {
			if (refMetaDataService != null) {
				...
				if (null != query) {
					...
					List<Object> dataListMap = new ArrayList<Object>();
					for (JSONObject jsonObject : dataList) {
						jsonObject.element("custtype", jsonObject.get("customerOrEnterprise"));
						//企业用户转
						if (null != jsonObject.get("customerOrEnterprise")&&jsonObject.get("customerOrEnterprise").equals("2")){
							//如果企业客户，转赋值客户名字
							jsonObject.element("enterpriseName", jsonObject.get("userName"));
							//企业个人公用一个字段，这里预防vm 界面有引用企业的，将个人的赋值给企业
							jsonObject.element("enterprisePersonIdentificationNo", jsonObject.get("userIdentificationNo"));
						}else{
							jsonObject.element("enterpriseName", jsonObject.get("userName"));
							jsonObject.element("enterprisePersonIdentificationNo", jsonObject.get("userIdentificationNo"));
						}
