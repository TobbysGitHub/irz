...
var HIGHLIGHT_COLOR = Color.get(255,225,225);
var HIGHLIGHT_STROKE_COLOR = Color.get(165,5,0);
var CURRENT_COLOR = Color.get(240,252,225);
...
var APPROVEUSERTASK_IMAGE = 		"images/deployer/approveuser.png";
...
var MESSAGE_COMPENSATE_IMAGE = 	"images/deployer/forcompensation.png";
var MESSAGE_COMPENSATE_THROW_IMAGE="images/deployer/forcompensation.png";
var EXCLUSIVE_GATEWAY_IMAGE= "images/deployer/gateway/exclusive.png";
var COMPLEX_GATEWAY_IMAGE= "images/deployer/gateway/complex.png";
var EVENT_GATEWAY_IMAGE= "images/deployer/gateway/event.png";
var INCLUSIVE_GATEWAY_IMAGE= "images/deployer/gateway/inclusive.png";
var PARALLEL_GATEWAY_IMAGE= "images/deployer/gateway/parallel.png";
...
// var DefaultProcessDiagramCanvas = {
	...
	*/
	...
	onHoverIn: function(event, instance, element){
	  ...
	  border.attr({"stroke-width": this.strokeWidth + 1, 
			"stroke": Color.get(95,170,240),
			"opacity": 1,
			"stroke-dasharray":null});
	 ...
	 onHoverOut: function(event, instance, element){
	   ...
	   var left=parseInt(element[0].getAttribute("x"));
	   var right=parseInt(element[0].getAttribute("x"))+parseInt(element[0].getAttribute("width"));
	   
	   var bottom=parseInt(element[0].getAttribute("y"));
	   var top=parseInt(element[0].getAttribute("y"))+parseInt(element[0].getAttribute("height"));
	   
	   // provide callback
	   if (ProcessDiagramGenerator.options && ProcessDiagramGenerator.options.on && ProcessDiagramGenerator.options.on.out
			   &&!(parseInt(event.clientX)>=left&&parseInt(event.clientX)<=right&&parseInt(event.clientY+document.body.scrollTop)>=bottom&&parseInt(event.clientY+document.body.scrollTop)<=top)) {
	     var args = [instance, element, contextObject];
	 ...
	 addHandlers: function(set, x, y, width, height, type){
	   ...
	   if (type.indexOf("event")!=-1 ) {
	     ...
	     var border = this.g.rect(x - 4, y - 4, width+9, height+9, TASK_CORNER_ROUND+2);
	     var overlay = this.g.rect(x, y, width, height, TASK_CORNER_ROUND);
	   }
	   border.attr({stroke: Color.get(132,112,255)/*Color.Tan1*/,"stroke-width": 1, opacity: 0.0});
	   border.id = contextObject.id + "_border";

	   set.push(border);

	   overlay.attr({stroke: Color.Orange,"stroke-width": 1, fill: Color.Green, opacity: 0.0, cursor: "hand"});
	...
	_drawExclusiveGatewayImage: function(x, y, width, height, image){

		var img = this.g.image(image, x, y, width, height);
	},
	...
	_drawNoneEndEvent: function(x, y, width, height, image, type) {
		...
		outerCircle.attr({"stroke-width": this.strokeWidth*2,
						"stroke": strokeColor,
						"fill": fillColor});
	...
	drawCatchingCompensateEvent: function(x, y, width, height, isInterrupting, name) {
		  this.g.setStart();
			this._drawCatchingEvent(x, y, width, height, isInterrupting, null);
			
			this._drawCatchingEventImage(x, y, width, height, MESSAGE_COMPENSATE_IMAGE);
	...
	drawThrowingCompensateEvent: function(x, y, width, height, name) {
		  this.g.setStart();
			this._drawCatchingEvent(x, y, width, height, null, null);
			
			this._drawCatchingEventImage(x, y, width, height, MESSAGE_COMPENSATE_THROW_IMAGE);
...
// TODO: generate polylineId or do something!!
       ...
       /* var flowName = contextObject.name;
		...
		var polyCloneAttrNormal = {"stroke-width": this.getStroke() + 5, stroke: Color.get(132,112,255), opacity: 0.0, cursor: "default"};
	...
	_drawArrowHead: function(line, connectionType){
		...
		if(version.length>1)
		{
			...
			{ 
				if(Math.abs(line.y2-line.y1)<2)
					line.y2=line.y2-ARROW_WIDTH/2;
				if(Math.abs(line.x2-line.x1)<2&&line.y2<line.y1)
					line.x2=line.x2-ARROW_WIDTH/2;
			} 
		}
	...
	*/
	...
	_drawTask: function(name, x, y, width, height, thickBorder) {
		...
		var attr = {stroke: TASK_STROKE_COLOR, fill: this.getPaint()};
		shape.attr(attr);
	...
	*/
	...
	drawParallelGateway: function(x, y, width, height) {
	  this.g.setStart();
		...
		this._drawExclusiveGatewayImage(x, y, width, height, PARALLEL_GATEWAY_IMAGE);
	...
	drawExclusiveGateway: function(x, y, width, height) {
	  this.g.setStart();
		...
		this._drawExclusiveGatewayImage(x, y, width, height, EXCLUSIVE_GATEWAY_IMAGE);
	...
	drawInclusiveGateway: function(x, y, width, height){
	  this.g.setStart();
		...
		this._drawExclusiveGatewayImage(x, y, width, height, INCLUSIVE_GATEWAY_IMAGE);
	...
	drawEventBasedGateway: function(x, y, width, height){
	  this.g.setStart();
		...
		this._drawExclusiveGatewayImage(x, y, width, height, EVENT_GATEWAY_IMAGE);
		
		var set = this.g.setFinish();
    this.addHandlers(set, x, y, width, height, "gateway");
	},
	drawForCompensationMarker: function( x, y, width, height) {
		this.g.image(MESSAGE_COMPENSATE_IMAGE, x+width/2+5, y+height-15, 15, 15);
	},
	...
	*/
	...
	_drawMultilineText: function(text, x, y, boxWidth, boxHeight, verticalAlign, horizontalAlign) {
		if (!text || text == "") 
			return;
		var number=Math.ceil(boxWidth/13);
		var size=text.length/number;
		y=y+(boxHeight/2-12);
		if(boxHeight<Math.ceil(size)*12)
			y=y-(Math.ceil(size)*12-boxHeight)/2;
		...
		{
			tempString=text.substring(j*number,number*(j+1));
			var measurer = new LineBreakMeasurer(this.g, x, y, tempString, font);
