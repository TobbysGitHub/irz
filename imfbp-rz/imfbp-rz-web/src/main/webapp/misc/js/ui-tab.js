...
function setHeight() {
	...
	//jQuery("#tab-content").css("margin-top", 9);//原代码
	jQuery("#tab-content").css("margin-top", 23);//为调整首页页签展示位置修改后代码
	jQuery("#tab-content .auto-content-height").each(function() {
		jQuery(this).css('padding', 0).height(height - 45);
	});
}
/*获取顶部选项卡总长度*/
function tabNavallwidth() {
	var taballwidth = 0,
		$tabNav = jQuery("#tabNav").find(".ui-tabs-nav"), //ul
		$tabNavWp = jQuery("#tabNav").find("#tabs2"), //ul 父级容器
		$tabNavitem = jQuery("#tabNav").find(".ui-tabs-nav li"), // ul  li
		$tabNavmore = jQuery("#tabNav").find(".Hui-tabNav-more"); // 更多 < >
	if(!$tabNav[0]) {
		return
	}
...
function Hui_admin_tab(obj) {
	if(jQuery(obj).attr('module_url') || obj.module_url) {
		...
		//首页更多打开新页签传参数type消息类型
		var type = obj.type;
		//首页查询全部个人客户或者企业客户
		var flag = obj.flag;
		//首页查询一段时间内的贷款申请或者贷款合同
		var timeBunket = obj.timeBunket;
		
		var _titleName = jQuery.trim(_title.replaceAll('&nbsp;',''));
		...
		var url = _href+"?moduleValue="+_value+"&rt="+randomStr(16);
		//billid
		...
		//首页查询一段时间内的贷款申请或者贷款合同
		if(timeBunket!=undefined&&timeBunket!=""){
			url = url+timeBunket;
		}
		if(!bStop) {
			creatIframe(_href, _titleName, _value, _param,type,flag,timeBunket);
		} else {
			
			jQuery("#tabs-"+(bStopIndex+1)).find("iframe").attr("src", url);
...
function creatIframe(href, titleName, moduleValue, param,type,flag,timeBunket) {
